...
int EncodeVarint32(char* buf, uint32_t value) {
    int len = 0;
    while (value >= 128) {
        buf[len++] = (value & 0x7f) | 0x80;  // 最高位设为1，表示还有后续字节
        value >>= 7;
    }
    buf[len++] = value & 0x7f;  // 最后一个字节，最高位为0
    return len;
}
...
int DecodeVarint32(const char* buf, uint32_t* value) {
    *value = 0;
    ...
    while (len < 5) {  // 最多5个字节
        unsigned char byte = buf[len++];
        *value |= static_cast<uint32_t>(byte & 0x7f) << shift;
        
        if ((byte & 0x80) == 0) {
            return len;  // 最高位为0，解码完成
        }
...
bool DecodeString(const char** buf, const char* limit, std::string* value) {
    uint32_t len;
    int consumed = DecodeVarint32(*buf, &len);
    
    if (consumed < 0 || *buf + consumed + len > limit) {
        return false;  // 长度错误或超出范围
    }
...
void PutVarint32(std::string* dst, uint32_t value) {
    char buf[5];  // 最多5字节
    int len = EncodeVarint32(buf, value);
    dst->append(buf, len);
}
...
uint32_t ValueCRC32(uint32_t icrc, const char* data, size_t n) {
    ...
    for (size_t i = 0; i < n; i++) {
        crc = ((crc << 8) & 0xffffff00) ^ crctab[((crc >> 24) & 0xff) ^ p[i]];
    }
    return crc;
}
