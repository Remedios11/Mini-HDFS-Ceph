...
bool DataNodeManager::RegisterDataNode(const DataNodeInfo& info) {
    ...
    new_info.status            = DataNodeStatus::ALIVE;
    new_info.last_heartbeat    = NowMs();
...
bool DataNodeManager::HandleHeartbeat(const std::string& id,
                                       int64_t free_space,
                                       int32_t block_count) {
    std::lock_guard<std::mutex> lock(mutex_);
...
std::vector<DataNodeInfo> DataNodeManager::GetAliveDataNodes() const {
    ...
    for (const auto& [id, info] : nodes_) {
        if (info.status == DataNodeStatus::ALIVE) result.push_back(info);
    }
    return result;
}

std::optional<DataNodeInfo> DataNodeManager::GetDataNode(const std::string& id) const {
    std::lock_guard<std::mutex> lock(mutex_);
    auto it = nodes_.find(id);
...
void DataNodeManager::CheckDataNodeHealth() {
    ...
    for (auto& [id, info] : nodes_) {
        ...
        if (elapsed > timeout_ms) {
            if (info.status != DataNodeStatus::DEAD) {
...
size_t DataNodeManager::AliveCount() const {
    ...
    size_t count = 0;
    for (const auto& [id, info] : nodes_)
        if (info.status == DataNodeStatus::ALIVE) count++;
    return count;
}
