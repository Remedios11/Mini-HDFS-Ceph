...
DataNodeServer::DataNodeServer(const std::string& data_dir,
    ...
    nn_client_        = std::make_unique<DataNodeClient>(
                            dn_id, nn_host, nn_port, block_store_.get());
}

DataNodeServer::~DataNodeServer() { Stop(); }
...
DataNodeResponse DataNodeServer::HandleWriteBlock(const WriteBlockRequest& req) {
    DataNodeResponse resp;
    auto write_resp = pipeline_handler_->HandleWrite(req);
...
DataNodeResponse DataNodeServer::HandleReadBlock(const ReadBlockRequest& req) {
    DataNodeResponse resp;
    std::string data;
    bool ok = block_store_->ReadBlock(req.block_id(), req.offset(),
                                       req.length(), &data);
    resp.set_success(ok);
    if (ok) {
